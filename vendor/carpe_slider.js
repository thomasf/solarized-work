...
(function () {
    ...
    CARPE.Sliders = {
        ...
        make: function (elmnt, params, DOMLocation) {
            ...
            CARPE.Sliders.elements.concat(slider);
            return slider;
        }
    };
}());
...
CARPE.Slider.prototype = {
    ...
    start: function (evnt) {
        ...
        if (this.enabled()) {
            var add = CARPE.addEventListener,
                doc = window.document;
            this.startOffsetX = this.x - evnt.screenX; // Horizontal
            // slider-mouse offset at start of slide.
            this.startOffsetY = this.y - evnt.screenY; // Vertical slider-mouse
            // offset at start of slide.
            this.panel.focus();
            this.sliding = true;
    ...
    jump: function (evnt) {
        'use strict';
        evnt = evnt || window.event; // Get the mouse event causing the
        // slider activation.
        var scroll,
            pos,
            border = {},
    ...
    val: function () {
        ...
        var v = this.vertical,
            ...
            val = this[dir] * (1 - 2 * v) * this.scale + this.from * (!v) +
                    this.to * v;
        this.valueElmnt.value = this.value = CARPE.roundToDec(val,
                this.decimals, this.zerofill);
        return this;
    },
    ...
    updateTargets: function () {
        ...
        if (targets && targets.length) {
            len = targets.length;
            for (i = 0; i < len; i += 1) {
                if (targets[i].setValue &&
                        CARPE.isFunction(targets[i].setValue)) {
                    targets[i].setValue(val);
                } else if (Object.prototype.hasOwnProperty.call(targets[i],
                        'value') || (targets[i].constructor.toString().
                        toLowerCase().indexOf('input') >= 0)) {
                    targets[i].value = val;
                } else if (CARPE.isFunction(targets[i])) {
                    targets[i](val);
                } else {
                    targets[i] = val;
                }
